...
const Row = ({ row, seats, handleSeatClick, getSeatStyle }) => {
  ...
  return (
    <div className="flex justify-between sm:justify-around items-center">
      <div className="sm:w-6 text-xs md:text-sm text-center text-muted-foreground font-semibold">
        {row}
      </div>
        ...
        <div className="flex gap-2">
          {rowSeats.slice(0, 6).map((seat) => (
            <button
              ...
              className={cn(
                "w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-6 md:h-6 lg:w-10 lg:h-10 rounded border sm:border-2 text-[8px] sm:text-[10px] md:text-xs lg:text-lg font-medium transition-all hover:scale-105",
                getSeatStyle(seat)
              )}
